...
describe('AddMovieGatewayImpl',()=>{
  it('should send a movie to the queue',()=>{
    ...
    let amqpClient:jest.Mocked<AmqpClient> = {
      sendToQueue: jest.fn(),
    } as unknown as jest.Mocked<AmqpClient>;
    let AddMovieGateway:AddMovieGateway = new AddMovieGatewayImpl(amqpClient);
    AddMovieGateway.addMovie(movie);
    expect(amqpClient.sendToQueue).toHaveBeenCalledWith(MOVIE_QUEUE,JSON.stringify(movie));
  });
});